...
highlighted_block_pos = None  # (x, y) of block being hovered
...
def update_world_generation():
    ...
    global world_generation_progress, world_generation_status, game_state, pending_multiplayer_world_name
    ...
    if world_generation_progress >= world_generation_total:
        ...
        if 'pending_multiplayer_world_name' in globals() and pending_multiplayer_world_name:
            ...
            # Get the server that was created earlier
            lan_server = multiplayer_ui.get_lan_server() if multiplayer_ui else None
            
            if lan_server:
                # Now start the server with the generated world
                server_world_data = {
...
def break_block(mx, my):
    """Break blocks instantly - back to normal"""
...
def update_block_highlight(mx, my):
    """Update which block is highlighted based on mouse position"""
    global highlighted_block_pos
    
    # Convert mouse position to block coordinates
    bx = int((mx + camera_x) // TILE_SIZE)
    by = int((my + camera_y) // TILE_SIZE)
    
    # Get the block at this position
    block = get_block(bx, by)
    
    # Only highlight if there's an actual block (not air/sky)
    if block and block != "air":
        highlighted_block_pos = (bx, by)
    else:
        highlighted_block_pos = None

def draw_block_highlight():
    """
    Draw highlight overlay on the block under the mouse cursor.
    
    ⚠️ IMPORTANT: Call this in the main game rendering loop after blocks are drawn.
    Also call update_block_highlight(mx, my) every frame to update the highlight position.
    """
    global highlighted_block_pos
    
    if highlighted_block_pos is None:
        return
    
    bx, by = highlighted_block_pos
    ...
    # Draw a bright outline around the highlighted block
    highlight_rect = pygame.Rect(screen_x, screen_y, TILE_SIZE, TILE_SIZE)
    pygame.draw.rect(screen, (255, 255, 255), highlight_rect, 3)  # White outline
    
    # Draw a subtle semi-transparent overlay
    highlight_surface = pygame.Surface((TILE_SIZE, TILE_SIZE), pygame.SRCALPHA)
    highlight_surface.fill((255, 255, 255, 30))  # Semi-transparent white
...
def load_world_data():
    ...
    try:
        # CRITICAL: Clear old world data completely before loading new world
        print(f"🧹 Clearing old world data (had {len(world_data)} blocks)")
        world_data.clear()
        entities.clear()
        dropped_items.clear()
        crops.clear()
        ...
        new_entities = world_system.current_world_data.get("entities", [])
        entities.extend(new_entities)  # Use extend to modify the global list
        
        new_dropped_items = world_system.current_world_data.get("dropped_items", [])
        dropped_items.extend(new_dropped_items)  # Use extend to modify the global list
...
pending_multiplayer_world_name = None  # World name for multiplayer server after generation
is_multiplayer_host = False  # Whether this game is hosting a server
is_multiplayer_client = False  # Whether this game is connected as a client
            ...
            if game_state == GameState.WORLD_NAMING:
                ...
                if merchant_shop_open:
                    ...
                    if multiplayer_ui:
                        ...
                        if lan_server or lan_client:
                            # Multiplayer game - show confirmation or just quit
                            print("🌐 Leaving multiplayer game...")
            ...
            if game_state == GameState.GAME:
                ...
                if multiplayer_ui:
                    ...
                    if action == "host":
                        ...
                        if world_name:
                            print(f"🌐 Starting LAN server with NEW world: {world_name}")
                            ...
                            try:
                                ...
                                # ALWAYS create a NEW world for multiplayer
                                print(f"🌍 Generating fresh world for multiplayer...")
                                
                                # Create and start LAN server (will be started after world gen)
                                ...
                                print(f"🎮 World generation started, server will start when ready")
                        ...
                        if server_info:
                            ...
                            try:
                                ...
                                if lan_client.connect_to_server(server_ip, server_port):
                                    ...
                                    import time
                                    print("⏳ Waiting for world data from server...")
                                    time.sleep(1.0)  # Give more time for server response
                                    ...
                                    if lan_client.world_data:
                                        ...
                                        print(f"🎮 Joined as {client_username}")
                                    else:
                                        print("⚠️ No world data received from server")
                                        lan_client.disconnect()
                                else:
