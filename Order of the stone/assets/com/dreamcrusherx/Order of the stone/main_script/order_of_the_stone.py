...
class InfiniteWorldGenerator:
    def __init__(self, seed: int = None):
        ...
        self.generated_chunks: set = set()
        print(f"🌍 Infinite World Generator initialized with seed: {seed}")
    
    def get_chunk_coordinates(self, world_x: int, world_y: int, chunk_size: int = 16):
    ...
    def generate_chunk(self, chunk_x: int, chunk_y: int, chunk_size: int = 16):
    ...
    def _generate_terrain_at(self, x: int, y: int):
    ...
    def generate_structures_in_chunk(self, chunk_x: int, chunk_y: int, chunk_size: int = 16):
...
class GameConfig:
    ...
    max_world_height: int = 4000  # 4000 blocks build limit
...
def generate_terrain_column(x):
    ...
    # Stone layer (200 blocks deep - reduced from 4000)
    for y in range(ground_y + 1 + dirt_depth, ground_y + 1 + dirt_depth + 200):
        set_block(x, y, "stone")
    
    # Bedrock at bottom of 200-block stone layer
    bedrock_y = ground_y + 1 + dirt_depth + 200
