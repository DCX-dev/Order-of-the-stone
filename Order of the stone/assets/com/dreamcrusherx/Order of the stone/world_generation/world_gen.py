...
class WorldGenerator:
    ...
    def _generate_simple_terrain(self, blocks: Dict[str, str], world_width: int):
        ...
        for x in range(-world_width//2, world_width//2):
            ...
            # Generate terrain column with 200-block stone layer
            for y in range(height, height + 200 + 15):  # 200 blocks of stone + 15 blocks for surface layers
                if y == height:
                    blocks[f"{x},{y}"] = "grass"
                elif y < height + 4:
                    blocks[f"{x},{y}"] = "dirt"
                elif y < height + 200 + 4:  # 200 blocks of stone
                    blocks[f"{x},{y}"] = "stone"
                else:
                    blocks[f"{x},{y}"] = "bedrock"  # Bedrock at bottom of 200-block stone layer
